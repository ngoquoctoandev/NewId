    ...
    INewIdFormatter
{
    ...
    public unsafe string Format(in byte[] bytes)
    {
        ...
        if (Avx2.IsSupported && BitConverter.IsLittleEndian)
        {
            ...
            return string.Create(_length, (bytes, isUpperCase, _prefix, _suffix), (span, state) => { EncodeVector256(span, state); });
        }
#endif
    ...
    private static void EncodeVector256(Span<char> span, (byte[] bytes, bool, char _prefix, char _suffix) state)
    {
        ...
        var swizzle = Vector256.Create(0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
        ...
        var shuffleSpare = Avx2.Shuffle(hexVec, Vector256.Create(14, 0xFF, 15, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 12, 0xFF, 13, 0xFF, 14, 0xFF, 15, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF));
