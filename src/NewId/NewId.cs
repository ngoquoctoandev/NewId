...
using FSH.NewId.NewIdFormatters;
using FSH.NewId.NewIdProviders;
    ...
    IFormattable
{
    ...
    public bool Equals(NewId other) => other._a == _a && other._b == _b && other._c == _c && other._d == _d;
    ...
    public static bool operator ==(in NewId left, in NewId right) => left._a == right._a && left._b == right._b && left._c == right._c && left._d == right._d;

    public static bool operator !=(in NewId left, in NewId right) => !(left == right);

    public static bool operator <(in NewId left, in NewId right) => left.CompareTo(right) < 0;
    ...
    public static NewId Next() => _getGenerator().Next();
    ...
    /// <returns></returns>
    public static ArraySegment<NewId> Next(NewId[] ids, int index, int count) => _getGenerator().Next(ids, index, count);

    /// <summary>
    ///     Generate a NewId, and return it as a Guid
    /// </summary>
    /// <returns></returns>
    public static Guid NextGuid() => _getGenerator().NextGuid();

    /// <summary>
    ///     Generate a NewId, and return it as a Guid in sequential format
    /// </summary>
    /// <returns></returns>
    public static Guid NextSequentialGuid() => _getGenerator().NextSequentialGuid();
...
#if NET6_0_OR_GREATER
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void FromByteSpan(ReadOnlySpan<byte> span, out NewId newId)
    {
...
#if NET6_0_OR_GREATER
    private static void FromSequentialByteSpan(ReadOnlySpan<byte> span, out NewId newId)
    {
        Debug.Assert(span.Length == Unsafe.SizeOf<Guid>());
        Debug.Assert(Ssse3.IsSupported && BitConverter.IsLittleEndian);
